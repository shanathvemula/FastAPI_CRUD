...
class UserCreate(BaseModel):
    ...
    @root_validator()
    def validating_username_phoneno_email(cls, field_values):
        if field_values['username'] or field_values['phoneno'] or field_values['email']:
            if not list(DBClient[DBName][UserCollection].find({'$or':[{'username':field_values['username']},{'phoneno':field_values['phoneno']},{'email':field_values['email']}]})):
                return field_values
